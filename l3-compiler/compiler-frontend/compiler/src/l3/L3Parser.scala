...
object L3Parser extends TokenParsers {
  ...
  private def sFun(args: List[String], body: Tree): Tree = {
	  val funcName = freshName("sFun")
	  LetRec(List(FunDef(funcName, args, body)), Ident(funcName))
  }
  private def sLet_*(bdgs: List[(String,Tree)], body: Tree): Tree = {
	  if (bdgs.length == 1)
	 	  Let(bdgs, body)
	  else
	 	  Let(List(bdgs.head), sLet_*(bdgs.tail, body))
  }
  private def sBegin(exprs: List[Tree]): Tree = {
    if (exprs.length == 1)
    	exprs.head
    else
    	Let(List((freshName("sLet"), exprs.head)), sBegin(exprs.tail))
  }
  private def sRec(name: String, bdgs: List[(String, Tree)], body: Tree) = 
	  LetRec(
	 		  List(FunDef(name, bdgs.map(x => x._1), body)),
	 		  App(Ident(name), bdgs.map(x => x._2))
	 		)
  private def sAnd(e1: Tree, e2: Tree): Tree = If(e1, e2, Lit(BooleanLit(false)))
  private def sOr(e1: Tree, e2: Tree): Tree = {
	  val valName = freshName("sOr")
	  Let(List((valName, e1)), If(Ident(valName), Ident(valName), e2))
  }
  ...
  private def sCond(branches: List[(Tree, Tree)]): Tree = {
	  if (branches.length == 1)
	 	  If(branches.head._1, branches.head._2, Lit(UnitLit))
	  else
	 	  If(branches.head._1, branches.head._2, sCond(branches.tail))
  }
  ...
  private def TODO[T]: T = error("not yet implemented")
}
